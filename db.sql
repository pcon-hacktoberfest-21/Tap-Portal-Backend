...
CREATE TABLE `STUDENT` (
  ...
  `Name` varchar(255) DEFAULT NULL,
  `Branch` varchar(255) DEFAULT NULL,
  `Tenth` float(5,3) DEFAULT NULL,
  `Twelfth` float(5,3) DEFAULT NULL,
...
CREATE TABLE `APPLICATION`(
  ...
  `Status` varchar(255) NOT NULL,
  PRIMARY KEY (`Id`),
  CONSTRAINT FOREIGN KEY(`COMPANY_ID`) REFERENCES `COMPANIES`(`Id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY(`Email`) REFERENCES `ALL_USER`(`EMAIL`) ON DELETE CASCADE ON UPDATE CASCADE
);